...
function buildAlignedSamples (input, filters = {}) {
  ...
  const rateOfTurn = normalizeSeries(input.rateOfTurn)
  const aligned = []
  const rejected = {
    missing: 0,
    slow: 0,
  ...
  for (const headingSample of heading) {
    ...
    if (sogSample.value < effective.minSog) {
      rejected.slow += 1
      continue
    }

    const rotSample = nearestSample(rateOfTurn, headingSample.t, maxGapMs)
    if (rotSample && Math.abs(radToDeg(rotSample.value)) > effective.maxRateOfTurn) {
      rejected.turning += 1
      continue
    }
    ...
    if (previousAccepted) {
      ...
      if (cogRateDeg > effective.maxCogRate) {
